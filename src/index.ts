...
export class MyMCP extends McpAgent {
	...
	async init() {
		...
		this.server.tool(
			"業務プロセスからBPMN2.0のXMLを生成する",
			{ instructions: z.string() },
			async ({ instructions }) => {
				// Full BPMN style guide content (simulated - in a real scenario, this might be fetched or loaded)
				const fullBpmnStyleGuide = `
これはBPMN2.0のスタイルガイドです。

あなたは BPMN 2.0 に準拠したプロセスモデリングAIです。

業務プロセスがユーザーから自然言語で与えられる。
\`## BPMN作成プロセス\`に従って、自然言語で与えられた指示を構文的に正しい \`.bpmn\` ファイル（XML形式）を作成する。

\`.bpmn\` ファイルを新規に作成するときの作成場所は次の優先順位に従う
1. 指示内で指定されているディレクトリ
2. 業務プロセスがファイルで引用されている場合は、当該コンテキストファイルと同じディレクトリ
3. ワークスペースのルートディレクトリ

## BPMN作成プロセス
1. \`## BPMN2.0の仕様書\`を読み込む
2. \`## スタイルガイド\`を読み込む
3. プールを描き、プロセス名のラベルを描いてく
4. 登場人物を考え、レーンを描き、レーンに役割名のラベルを書く
5. 開始イベントを描き、適切なラベルを書く
6. レーンにタスクを描き、シーケンスフローで業務の順番に接続する
7. 必要なゲートウェイと条件ラベルを描く
8. 終了ラベルを描き、終了状態のラベルを書く
...
// ... (The rest of the style guide, including レベル2, etc., would be included here)
// For brevity in this example, the full text is truncated.
// In a real implementation, the entire content fetched from the rule would be used.
`;

				const promptForLLM = `以下の業務プロセス記述に基づき、BPMN 2.0 形式のXMLを生成してください。
...
生成するXMLは、上記の指示およびスタイルガイドの全文を厳密に守ってください。
特に、BPMNの要素はスタイルガイドに記載されている「レベル1 記述適合サブクラス」に含まれるもののみを使用してください。
`;
				return {
					content: [{ type: "text", text: promptForLLM }],
				};
			}
		);
	}
